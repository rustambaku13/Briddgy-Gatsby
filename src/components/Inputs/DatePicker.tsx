...
export const DatePicker = chakra(
  ({
    ...
    if (oneWay) {
      ...
      if (selectedDate) from = moment(selectedDate).format("YYYY-MM-DD")
    } else {
      if (selectedDayRange.from)
        from = moment(selectedDayRange.from).format("YYYY-MM-DD")
      if (selectedDayRange.to)
        to = moment(selectedDayRange.to).format("YYYY-MM-DD")
    }
    return (
      <Box
        pos="relative"
